...
🚨 **書き方の罠が2つある**（2026-08-15 実測で確認）:

1. **ブロックの中では Markdown のリンク記法が効かない。**
   `[出典](https://...)` と書いても**そのままの文字列**になり、リンクにならないので
   検査が「出典のリンクがありません」で止まる。
   **生のHTMLで `<a href="https://...">○○</a>` と書く。**
   （生HTMLブロックの中は Markdown の処理が走らないため）
2. **4スペース以上インデントしない。**
   Markdown がコードブロックと解釈して、ブロックごと無効になる（divですらなくなる）。
   **行頭から書く。**
