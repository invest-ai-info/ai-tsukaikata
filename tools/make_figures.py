...
def installment_scale_hit_and_miss_chart() -> None:
    """予約件数を3倍にしても入金照合の誤判定が増えないかを実測（2026-09-20）。

    材料2本（整体サロン・音楽教室）×条件2水準（小規模12件／大規模36件、分割件数は
    3倍）×各3回＝12回の基本実験に、行番号を書かせる追加検証（大規模のみ3回）と
    二重計上の自己申告有無を合わせた。判定は docs/evidence/
    installment-scale-does-not-increase-misses.md の judge2.py 出力から。
    """
    _hit_and_miss_rows_chart(
        "installment-scale-hit-and-miss.svg",
        "件数を3倍にしても、誤判定は0のまま動かなかった",
        "実測2026-09-20。整体サロン・音楽教室の架空データ、小規模(12件)/大規模(36件)を各3回×2種＝12回。",
        "すべて「起きた回数」（少ないほど良い）。大規模は分割払いが3件→9件に増えている。",
        [
            ("小規模条件：本当の未入金1件を見落とした回数", 0, 6, "bad"),
            ("小規模条件：分割払いを誤って未入金扱いした回数", 0, 6, "bad"),
            ("大規模条件：本当の未入金3件のどれかを見落とした回数", 0, 6, "bad"),
            ("大規模条件：分割払いを誤って未入金扱いした回数", 0, 6, "bad"),
            ("同じ入金明細を2つの予約に使い回した回数（行番号指定・大規模3回）", 0, 3, "bad"),
            ("入金の二重計上を自己申告した回数（全12回）", 0, 12, "bad"),
        ],
        [
            "小規模も大規模も、見落とし・誤判定とも0回で差が出なかった。",
            "大規模データには「阪本様」と「坂本様」という似た氏名が偶然混在していたが、6回とも取り違えなかった。",
            "＝この材料・この回数の範囲では、件数を増やしても入金照合の精度は落ちなかった。",
        ],
        "予約件数を3倍にしても入金照合の誤判定が増えるかを実測した結果を示す表。すべて「起きた回数」で少ないほど良い。"
        "小規模条件（12件）で本当の未入金1件を見落とした回数は6回中0回、分割払いを誤って未入金扱いした回数も6回中0回。"
        "大規模条件（36件、分割件数は3件から9件に増加）で本当の未入金3件のどれかを見落とした回数も6回中0回、"
        "分割払いを誤って未入金扱いした回数も6回中0回。行番号まで指定して明細の使い回しを確認した回数（大規模のみ3回）でも"
        "使い回しは0回。入金の二重計上を自己申告した回数は全12回を通じて0回。下の枠には、小規模も大規模も見落とし・誤判定とも"
        "0回で差が出なかったこと、大規模データに偶然混在していた「阪本様」「坂本様」という似た氏名も6回とも取り違えなかったこと、"
