---
title: Opus 5.5 は同じ仕事が40%安い——ただし「考える」を切れなくなった
description: 2026年9月22日に出た Claude Opus 5.5 を、1つ前の Opus 5 と比べました。単価は20%下がり、公式は「ふつうの使い方で費用が40%減る」と説明しています。一方で、自動化を組んでいる人には動かなくなる設定が4つあります。数字は Anthropic・OpenAI・Google の公式ページのものだけです。
category: tools
scene: choose
...
Anthropic は 2026年9月22日に **Claude Opus 5.5** を発表しました（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。新しい「Claude 5.5」世代の最初のモデルです。

1つ前の Opus 5 との違いを3行にすると、こうなります。

- **値段が下がった。**入力は100万トークンあたり $5 から **$4**、出力は $25 から **$20** です（出典: <https://platform.claude.com/docs/en/about-claude/pricing>）。
- **同じ仕事の費用は、公式の説明で40%減。**単価が下がったうえに、1つの仕事に使う量も減ったためです（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。
- **自動化の設定が4つ動かなくなる。**<mark>「考える」を切る設定や、道具を強制する設定はエラーになります</mark>（出典: <https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5>）。

先に断っておきます。**この記事は運営者が Opus 5.5 を試した記録ではありません。**公式ページに書かれていることを読んで整理したものです。

Opus 5 の記事は [Claude Opus 5 は何が変わったのか](/tools/claude-opus-5/) にあります。上位モデルの話は [Claude Fable 5.1 の記事](/tools/claude-fable-5-1/) に書いています。
...
| 学習データの締め切り | 2026年5月 | **2026年6月** |
| 「どれだけ考えるか」の既定値 | high | **medium** |
| モデルの呼び名（API） | `claude-opus-5` | `claude-opus-5-5` |

出典: 料金は <https://platform.claude.com/docs/en/about-claude/pricing>、Opus 5.5 の仕様は <https://platform.claude.com/docs/en/models/opus-5-5/overview>、Opus 5 の仕様は <https://platform.claude.com/docs/en/models/opus-5/overview>
...
**一番大きく下がったのは「読み直し」です。**$0.50 から $0.20 になり、公式は60%減と書いています。

読み直しとは、一度読ませた資料を次の質問でもう一度読むぶんの料金です。公式は、自動で長く作業させる使い方では、<mark>この読み直しが費用の大半を占める</mark>と説明しています（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。

### 「40%安い」の中身

公式の説明を分けると、安くなる理由は2つです。

1. **単価が下がった。**入力と出力で20%、読み直しで60%
2. **1つの仕事に使う量が減った。**公式は「トークンあたりも安く、1つの仕事あたりのトークンも少ない」と書いています

この2つを合わせて、<mark>既定の設定・ふつうの使い方で、Opus 5 より40%安い</mark>というのが公式の数字です（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。

<mark class="warn">40%は Anthropic 自身のテストの値です。自分の使い方で同じだけ下がるとは限りません。</mark>

公式が挙げた具体例は次のとおりです。すべて発表ページの記述です。
...
出力の速さについても、公式は「Opus 5 より30%以上速く書く」と説明しています（出典: 同上）。
...
- **測ったのは、ほぼ Anthropic 自身です。**AutomationBench だけは Zapier が測った値です（出典: 同上）。
- **点数は最大の設定（max）のものです。**既定の設定（medium）で使うと、この点数にはなりません。
- **安全装置が働いた課題は、別のモデルが解いています。**公式は「Opus 5.5 の点数を下げている可能性が高い」と注記しています（出典: 同上）。
- **公式自身が「点差ほど差は無い」と書いています。**<mark>社内で使うと、Opus 5.5 と Fable 5.1 の差は点数より小さい</mark>という説明です（出典: 同上）。

### 書く文章が読みやすくなった

チャットで使う人に一番効くのは、たぶんここです。公式は、Opus 5 への不満として多かったのが「文章の分かりにくさ」だったと認めています（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。

Opus 5.5 で変えたと公式が書いている点は4つです。
...
発表ページには、同じ依頼への Opus 5 と Opus 5.5 の返事が並べて載っています。たとえば「Slack のやり取りを上司向けに3点でまとめて」という依頼です。
...
### Claude の有料プランで使っている人に関係すること

発表ページには、チャットの利用者に関係する変更も2つ書かれています（出典: <https://www.anthropic.com/news/claude-opus-5-5>）。

- **5時間ごとの利用上限を引き上げる。**対象は Pro・Max・Team と、席ごとに契約する Enterprise
- **利用上限のリセットを1回もらえる。**好きなときに使えるよう、取っておくこともできる

引き上げ幅は発表ページに書かれていません。だからこの記事にも書けません。

## 自動化を組んでいる人が引っかかるところ

ここは API で自動化を組んでいる人向けです。Opus 5 で動いていた設定のうち、**4つがエラーになります**。すべて公式の「What's new」ページの記述です（出典: <https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5>）。

| 何が起きるか | Opus 5 | Opus 5.5 |
|---|---|---|
...
4. パソコン操作は新しい道具（`computer_toolset_20260801`）に移す

エラーにはならないけれど、気づきにくい変化も3つあります。
...
- **道具を使う合間のメモが画面に出なくなる。**合間の文章が「考える」側に入るため、途中経過を利用者に見せている仕組みは黙ったままになります
...
- **OpenAI と Google は、入力が長くなると単価が上がります。**Google は「20万トークンを超えたら」と明記しています。OpenAI の料金ページは「短い」「長い」の境目を書いていません（出典: 上の各料金ページ）。
...
- **API で Opus 5 を毎日使っている人。**<mark>単価が20%下がるので、同じ使い方なら支払いは確実に下がります</mark>
- **長い資料を何度も読ませる自動化を組んでいる人。**読み直しの料金が60%下がります
- **Claude の文章が回りくどいと感じていた人。**公式が一番力を入れて直したと書いている部分です

**急がなくていい人・先に確認が要る人**

- **「考える」を切る設定や、道具の強制指定を使っている人。**そのままではエラーになります
- **Opus 5 の答えの深さに満足している人。**既定値が medium に下がったので、同じ深さが欲しいなら `effort` を指定し直す必要があります
- **Sonnet や Haiku を使っている人。**公式は Sonnet 5.5 と Haiku 5.5 を「数週間のうちに」出すと書いています（出典: <https://www.anthropic.com/news/claude-opus-5-5>）
