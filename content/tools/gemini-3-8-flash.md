...
- DeepSWE v1.1・Vals Finance Agent V2・Harvey's Legal Agent Benchmarkでは「3.7 Flashや他の最先端モデルを上回る」と説明されています。ただし具体的な点数は本文に書かれていません（出典: 同上）。
...
そして、<mark>このCyber版には、通常モデルにあるはずの「モデルカード」も「料金ページへの掲載」もありません</mark>。実際に確認したところ、次のとおりでした。

- モデルカードのURL（`/models/model-cards/gemini-3-8-flash-cyber`）は404で存在しない
- 料金ページ（ai.google.dev）にも "cyber" の文字列は一度も出てこない
提供先は「信頼できる政府機関、重要インフラの運用者、ソフトウェア開発者」に限られます。申請フォームからの審査を通った組織だけが対象です（出典: 同上）。この記事を読んでいる大半の会社員は、このモデルを使うことができません。
