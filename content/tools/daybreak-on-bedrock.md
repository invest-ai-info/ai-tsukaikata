...
ただし、そこで終わりではありません。<mark>汎用の Sol が選べる一番安い Global 経路を、Daybreak Blue は選べません</mark>。汎用の Sol のモデルカードには Global 経路の行があって $5.00 / $30.00 です。一方、Daybreak の2つは Geo も Global も「Not supported」と書かれています（出典: 上記の3枚のモデルカード）。**同じ中身でも、安く済ませる選び方だけが最初から消えています。**
...
**データの扱い。**AWS のブログには、次のことが書かれています（出典: 上記の AWS ブログ）。

- 推論に使ったデータはモデルの学習には使わない
- OpenAI とのデータ共有に同意する必要はない
- 不正利用の検知で印が付いた通信だけを AWS が最大30日保持する（申請すれば保持しないようにできる）
