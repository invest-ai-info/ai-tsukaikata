...
- [ ] 執筆（記事の型は CLAUDE.md どおり: 5ブロック・マーカー上限・**図解・リスト・表・グラフを多く**＝
      図2枚以上・数値はグラフ・手順はリスト・比較は表・散文の塊を作らない〈2026-09-21 オーナー指示〉・座標は計算）
- [ ] 機械検査: `python -m pytest -q` ／ `python -m src.build` ／ `python tools/check_numbers.py <記事>`
- [ ] 🆕 読みやすさ: `python tools/check_readability.py <記事>`（60字以上の文が**15%以下**・120字以上は0）。🔑 直し方は「文を割る」より**「三つ以上並んでいるものを箇条書きにする」**が先
