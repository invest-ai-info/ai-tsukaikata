...
### 0. ⏰ 明朝のダイジェストに停止警告が出るか確認する（今日入れた機能の最終確認）

`hf-zhipu` が **2026-06-16 で47日更新なし**の状態で `seen.json` に記録済み。閾値30日を超えているので、**翌朝のダイジェストメールに停止警告が1件出るはず**。

- **出れば**: 停止検出が本番で効いた証拠。この項目は消してよい
- **出なければ**: `run_digest` → `stale_sources` → `build_body` のどこかが繋がっていない。単体テストは通っているので、疑うのは配線かワークフロー側
- ⚠️ ダイジェストは cron 上 07:22 JST だが**実測では8〜9時台にずれる**（下記「schedule は定刻に来ない」）。9時台まで来なくても異常ではない
