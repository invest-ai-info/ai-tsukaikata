...
**いま6本**（2026-08-02 に `verify-before-report` を追加）。ネタは `content/_ideas.md`。

⚠️ 数え方に注意。about / privacy は固定ページで審査の実質的な対象にならないので、**レシピ記事の本数で数える**こと。20本まであと14本。
...
| | |
|---|---|
| 生成 | `python -m src.build` → `build/` に14ファイル。`build/` は `.gitignore` 済み |
| プレビュー | `ai-tsukaikata-preview`（`http://localhost:8791`）。設定は `.claude/launch.json` |
| 記事 | レシピ6本 + about / privacy。5本は実運用中の自動化から、1本（`verify-before-report`）はその5本で効いた指示文の集約 |
| 図 | `static/images/` に18枚（手描きSVG）。記事に `<figure>` で埋め込み。`verify-before-report` だけ図なし |
| 記事の型 | **プロンプト中心**（2026-08-02 改訂）。プロンプト75個・コードブロック2個 |
...
`content/_ideas.md` に置いてある。AdSense申請は**記事20本を超えてから**なので、レシピ記事であと14本（いま6本）。

`_ideas.md` の筆頭候補だった「実際に動かして確かめてから報告して」の記事は **2026-08-02 に公開済み**（`verify-before-report`）。次はリストの残りから選ぶ。
